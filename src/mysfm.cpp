...
void savePLYFile(const std::vector<Landmark>& landmarks, const std::string& filename)
{
    ...
    // Write PLY header
    file << "ply\nformat ascii 1.0\n";
    file << "element vertex " << landmarks.size() << "\n";
    file << "property float x\nproperty float y\nproperty float z\n";
    file << "end_header\n";

    for (const auto& landmark : landmarks) {
        file << landmark.pt.x << " " << landmark.pt.y << " " << landmark.pt.z << "\n";
    }
...
// cv::Ptr<cv::DescriptorMatcher> matcher = cv::DescriptorMatcher::create(cv::DescriptorMatcher::FLANNBASED);
    ...
    savePLYFile(landmark, "pointcloud.ply");
}